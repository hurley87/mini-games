      ...
      scripts.forEach((script: Element, index: number) => {
        if (script.textContent) {
          ...
          if (jsErrors.length > 0) {
            warnings.push(`Script ${index + 1} has potential issues: ${jsErrors.join(', ')}`);
          }
        }
      });

    } catch (domError) {
      errors.push(`DOM parsing error: ${domError instanceof Error ? domError.message : 'Unknown error'}`);
    }
  ...
  try {
    ...
    if (!fixedHtml.includes('<body')) {
      ...
      if (headMatch) {
        ...
        if (htmlMatch) {
          fixedHtml = fixedHtml.replace(/<html[^>]*>/i, htmlMatch[0] + '\n<body>\n') + '\n</body>';
        }
      }
    ...
    return {
      ...
      warnings: [...revalidation.warnings, ...fixAttempts.map(attempt => `Auto-fix: ${attempt}`)]
    };

  } catch (error) {
    return {
      isValid: false,
      errors: [...validation.errors, `Fix attempt failed: ${error instanceof Error ? error.message : 'Unknown error'}`],
      warnings: validation.warnings
    };
  }
}
...
function validateJavaScript(jsCode: string): string[] {
  ...
  try {
    ...
    errors.push(`JavaScript validation error: ${error instanceof Error ? error.message : 'Unknown error'}`);
  }
...
function checkGameSpecificIssues(html: string): string[] {
  ...
  if (html.includes('window.awardPoints') && !html.includes('tryAwardPoints')) {
    warnings.push('Direct window.awardPoints usage detected - consider using tryAwardPoints helper');
  }
  
  // Check for external dependencies
  if (html.includes('src="http') || html.includes("src='http")) {
    warnings.push('External script/resource dependencies detected - may cause loading issues');
  }
